...
<Note>
  Este endpoint permite que você obtenha um token JWT para autenticar solicitações de API. O token deve ser incluído no cabeçalho de autorização de todas as solicitações subsequentes.
</Note>
